...
import { ArrowLeft, Printer, FileText, Edit, Clock, MessageSquareShareIcon, LucideContainer, CheckCircle, AlertTriangle, UserPlus } from "lucide-react";
...
export default function ViewDetailPurchaseOrder() {
    ...
    const handleFetchOrders = async () => {
        ...
        try {
            ...
            if (mappingData) {
                ...
                setRawData(data);
                setIsFree(mappingData?.estado === "libre");
            }
            return;
        } catch (error) {
    ...
    const handleAutoAsignarPedido = async (id) => {
        try {
            setLoading(true);
            setLoadingTitle("Autoasignar Pedido...");
            setError(null);
            setSuccess(null);
            let userId = localStorage.getItem("userId");
                            ...
                            <div className="d-flex justify-content-between align-items-center mb-4">
                                ...
                                <div className="d-flex gap-2">
                                    ...
                                    </Button>
                                    {
                                        isFree ? (
                                            <Button
                                                title="AutoAsignar Pedido"
                                                color="success"
                                                onClick={() => handleAutoAsignarPedido(pedido?.id)}
                                                disabled={loading}
                                            >
                                                <UserPlus size={18} className="me-2" /> Autoasignar
                                            </Button>
                                        ) :
                                            (
                                                <Button
